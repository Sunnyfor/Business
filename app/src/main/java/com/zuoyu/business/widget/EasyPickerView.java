...
/**
 * <pre>
 * Function：滚轮视图，可设置是否循环模式，实现OnScrollChangedListener接口以监听滚轮变化
 *
 * Created by JoannChen on 2017/7/4 15:48
 * QQ:411083907
...
//    private int textColor;// 颜色，默认Color.BLACK
    private int textPadding; // 文字之间的间隔，默认10dp
    private float textMaxScale;// 文字最大放大比例，默认2.0f
    private float textMinAlpha;// 文字最小alpha值，范围0.0f~1.0f，默认0.4f
    private boolean isRecycleMode; // 是否循环模式，默认是
    private int maxShowNum;// 正常状态下最多显示几个文字，默认3（偶数时，边缘的文字会截断）
    ...
    public EasyPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int textColor = a.getColor(R.styleable.EasyPickerView_epvTextColor, Color.BLACK);
